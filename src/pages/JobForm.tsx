...
const JobForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Upload resume using ApiService
      const { fileUrl } = await apiService.uploadResume(
        formData.resume,
        formData.email
      );

      // Submit application using ApiService
      await apiService.createApplicant({
        ...formData,
        resumeURL: `${API_BASE_URL}${fileUrl}`,
        sector: sector,
      });
